...
struct DailyService {
    ...
    static func getItems(for date: String) async throws -> [ToDoItemModel] {
        guard let collection = FirestorePath.users(userId)?.weekDays(date).items().asCollection() else {
    ...
    static func delete(_ item: ToDoItemModel) async throws {
        guard let document = FirestorePath.users(userId)?.weekDays(item.date).items(item.id).asDocument() else {
            throw FirebaseError.invalidPath
        }
